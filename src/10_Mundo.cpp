...
void Mundo::generarMundo(bool esSubterraneo) {
    for (int y = 0; y < alto; ++y) {
        for (int x = 0; x < ancho; ++x) {
            if (esSubterraneo) {
                // Si es el subterraneo, todo empieza lleno de Piedra
                cuadricula[y][x] = { TipoBloque::Piedra, true, 5, false };
            } else {
                // Si es la superficie, todo empieza lleno de Pasto por ahora
                cuadricula[y][x] = { TipoBloque::Pasto, false, 3, false };
            }
        }
    }
    std::cout << "¡Bloques base generados correctamente!" << std::endl;
}
    ...
    for (int y = 0; y < bloquesVisiblesY && y < alto; ++y) {
        for (int x = 0; x < bloquesVisiblesX && x < ancho; ++x) {
            ...
            if (cuadricula[y][x].tipo == TipoBloque::Pasto) {
                ...
                formaBlq.setFillColor(sf::Color(128, 128, 128)); // Gris
            } else {
                formaBlq.setFillColor(sf::Color::Black);
            }
